...
class TemporalConvNet(nn.Module):
    ...
    def __init__(self, num_inputs, num_channels, kernel_size=2, dropout=0.2):  # todo
        """
        Args:
        ...
        for i in range(num_levels):
            ...
            self.conv = nn.Conv2d(in_channels, out_channels, (1, kernel_size),
...
class ODEGCN(nn.Module):
    ...
    def forward(self, x):
        ...
        for blk in self.se_blocks:
            outs.append(blk(x))
        # len(outs) = 6
        # outs.shape = [16, 307, 12, 64]

        outs = torch.stack(outs)  # [6, 16, 307, 12, 64]
        x = torch.max(outs, dim=0)[0]  # [16, 307, 12, 64]
        x = x.reshape((x.shape[0], x.shape[1], -1))  # [16, 307, 768]

        return self.pred(x)  # [16, 307, 12]
